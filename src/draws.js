...
export function outsProbability(outs, boardLength) {
  ...
  if (
    ...
    outs < 0 ||
    outs > unseen ||
    ![3, 4].includes(boardLength)
  )
    throw new Error("Ungültige Trefferkarten.");
  return {
    next: outs / unseen,
    byRiver:
      boardLength === 3
...
export function drawOuts(hero, board) {
  ...
  for (const c of deck.filter((c) => !cards.includes(c))) {
    ...
    if (!straightHigh(mask) && straightHigh(mask | (1 << rank(c))))
      straight.push(c);
  }
  ...
  return {
    ...
    straight,
    union,
    ...outsProbability(union.length, board.length),
  };
}
